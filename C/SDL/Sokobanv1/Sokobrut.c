...
int loadMedia()
{
    ...
    if( mur == NULL )
    {
        printf( "Failed to load mur image! SDL Error: %s\n", SDL_GetError() );
        success = 0;
    }
...
int main(int argc, char *argv[])
{
	...
	else
	{
		...
		else
		{
			...
			SDL_BlitSurface( mur, NULL, screen, &positionMur );

			positionMur.x = positionMur.x + 34;
			...
			while ( continuer )
			{
				while ( SDL_PollEvent( &event ) != 0)
				{
					...
					case SDL_KEYDOWN:
						...
						case SDLK_UP:
							...
							positionMario.y = positionMario.y-3;
							break;
						case SDLK_DOWN:
							mario = marios[ KEY_PRESS_SURFACE_DOWN ];
							positionMario.y = positionMario.y+3;
						...
						case SDLK_LEFT:
							...
							positionMario.x = positionMario.x-3;
							break;
						case SDLK_RIGHT:
				...
				/*for( int i = 0; i < 408/34; i++)
				{
					for (int j = 0; j < 408/34; i++)
					{
						SDL_BlitSurface( mur, NULL, screen, &positionMur );
						positionMur.x = positionMur.x + 34;
					}
					positionMur.y = positionMur.y +34;
				}*/
